...
use core::panic;
use gltf::{
    accessor::{Accessor, DataType as GltfDataType},
...
fn access_bytes<'a>(gltf_bytes: &'a Vec<u8>, accessor: &'a Accessor) -> &'a [u8] {
...
impl AnimationChannel {
    ...
    pub fn property_at(&self, timestamp: f32) -> AnimatedProperty {
        ...
        let early_bound_frame = early_bound_frame.unwrap();
        let late_bound_frame = late_bound_frame.unwrap();
            ...
            if let Some(anim_name_candidate) = anim.name() {
                if anim_name_candidate == anim_name {
                    ...
                    for channel in anim.channels() {
                        ...
                        let sampler_input_bytes = access_bytes(&gltf_bytes, &sampler_input);
                        ...
                        let sampler_output_bytes = access_bytes(&gltf_bytes, &sampler_output);
